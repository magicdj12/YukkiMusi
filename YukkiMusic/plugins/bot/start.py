        ...
        if name[0:3] == "sud":
            ...
            if await is_on_off(config.LOG):
                ...
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} ربات اجرا شده چک کند  <code>لیست شده </code>\n\n**آیدی:** {sender_id}\n**نام کاربری:** {sender_name}",
                )
            return
        if name[0:3] == "lyr":
...
"""
            key = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(text="🎥 تماشا ", url=f"{link}"),
            ...
            if await is_on_off(config.LOG):
                ...
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    f"{message.from_user.mention} ربات اجرا شده تا چک کند <code> مشخصات ویدیو  </code>\n\n**آیدی کاربر:** {sender_id}\n**نام کاربر** {sender_name}",
                )
    else:
        try:
        ...
        if await is_on_off(config.LOG):
            ...
            return await app.send_message(
                config.LOG_GROUP_ID,
                f"{message.from_user.mention} شروع شده تا. \n\n**آیدی کاربری :** {sender_id}\n**نام کاربری:** {sender_name}",
            )
...
async def welcome(client, message: Message):
    ...
    if config.PRIVATE_BOT_MODE == str(True):
        if not await is_served_private_chat(message.chat.id):
            await message.reply_text(
                "حالت خصوصی این ربات فعال شده است و فقط صاحب من می‌تواند از آن استفاده کند. اگر می‌خواهید در چت خود از آن استفاده کنید، از صاحب من بخواهید چت شما را مجاز کند."
            )
