...
@app.on_message(command("SETTINGS_COMMAND",prefixes=["", "/"]) & filters.group & ~BANNED_USERS)
@language
...
"""✅<u>تنظیمات گروه:</u>
/settings - دریافت تنظیمات کامل گروه با دکمه‌های درون‌خطی

🔗 <u>گزینه‌ها در تنظیمات:</u>

1. می‌توانید کیفیت صدا را تنظیم کنید.
2. می‌توانید کیفیت ویدیو را تنظیم کنید.
3. **کاربران مجاز**: می‌توانید حالت دستورات ادمین را از اینجا به «همه» یا فقط «ادمین‌ها» تغییر دهید.
4. **حالت پاکسازی:** ربات پیام‌های خودش را بعد از ۵ دقیقه از گروه شما حذف می‌کند تا مطمئن شود چت شما تمیز و مرتب باقی می‌ماند.
5. **پاکسازی دستورات**: هنگامی که فعال است، ربات دستورات اجرا شده را بلافاصله حذف می‌کند.

<b><u>تنظیمات پخش:</u></b>
/playmode - دریافت پنل کامل تنظیمات پخش با دکمه‌هایی که می‌توانید تنظیمات پخش گروه خود را انجام دهید.

<b><u>گزینه‌ها در حالت پخش:</u></b>
1. **حالت جستجو** [مستقیم یا درون‌خطی] - حالت جستجوی شما را هنگام اجرای /playmode تغییر می‌دهد.
2. **دستورات ادمین** [همه یا ادمین‌ها] - اگر روی همه باشد، هر کسی در گروه شما می‌تواند از دستورات ادمین (مثل /skip، /stop و غیره) استفاده کند.
3. **نوع پخش** [همه یا ادمین‌ها] - اگر روی ادمین‌ها باشد، فقط ادمین‌های گروه می‌توانند موسیقی را در گفتگوی صوتی پخش کنند.
"""
