...
@app.on_message(command("QUEUE_COMMAND",prefixes=["", "/"]) & filters.group & ~BANNED_USERS)
@language
async def ping_com(client, message: Message, _):
    ...
    if "live_" in file:
        ...
        if videoid == "telegram":
            ...
            IMAGE = get_image(videoid)
    send = (
        "**⌛️ مدت:** زمان نامشخص\n\nروی کلید پایینی کلیک کنید تا لیست کامل را دریافت کنید"
        if DUR == "Unknown"
        else "\nروی کلید پایینی کلیک کنید تا لیست کامل را دریافت کنید."
    )
    cap = f"""**{app.mention} پخش کننده**
...
async def queue_back(client, CallbackQuery: CallbackQuery, _):
    ...
    if "live_" in file:
        ...
        if videoid == "telegram":
            ...
            IMAGE = get_image(videoid)
    send = (
        "**⌛️ مدت:** مدت زمان نامشخص\n\nروی کلید پایین کلیک کنید تا لیست کامل را دریافت کنید"
        if DUR == "Unknown"
        else "\nروی کلید پایین کلیک کنید تا لیست کامل را دریافت کنید."
    )
    cap = f"""**{app.mention} پخش کننده**
