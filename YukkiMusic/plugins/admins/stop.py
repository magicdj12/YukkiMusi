...
@app.on_message(command("STOP_COMMAND",prefixes=["", "/"]) & filters.group & ~BANNED_USERS)
    ...
    if not len(message.command) < 2:
        if extra_plugins_enabled:
            if not message.command[0][0] == "c" and not message.command[0][0] == "e":
                ...
                if deleted:
                    return await message.reply_text(f"**حذف فلتر {filter}.**")
                else:
                    return await message.reply_text("**فلتری پیدا نشد.**")
